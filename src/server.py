...
def main():
    ...
    earnapp_bonuses_total = Gauge("earnapp_bonuses_total", "Total earnings from referrals till date")
    earnapp_referral_part = Gauge("earnapp_referral_part", "Referral bonus percentage")

    earnapp_total_bandwidth_usage = Gauge("earnapp_total_bandwidth_usage", "Shows bandwidth usage of all devices combined")
    earnapp_device_bandwidth_usage = Gauge("earnapp_device_bandwidth_usage", "Unredeemed bandwidth usage", ["uuid"])
    earnapp_device_total_bandwidth_usage = Gauge("earnapp_device_total_bandwidth_usage", "Total bandwindth usage", ["uuid"])
    earnapp_device_redeemed_bandwidth = Gauge("earnapp_device_redeemed_bandwidth", "Redeemed bandwidth usage", ["uuid"])
    earnapp_device_rate = Gauge("earnapp_device_rate", "Price/GB of the device", ["uuid"])
    ...
    while True:
        try:
            ...
            for device in devices_infos.devices:
                earnapp_device_bandwidth_usage.labels(device.uuid).set(device.bandwidth_usage)
                earnapp_device_total_bandwidth_usage.labels(device.uuid).set(device.total_bandwidth_usage)
                earnapp_device_redeemed_bandwidth.labels(device.uuid).set(device.redeemed_bandwidth)
                earnapp_device_rate.labels(device.uuid).set(device.rate)
                earnapp_device_info.labels(device.uuid).info({'country': device.country, 'device_type': device.device_type})
